...
def get_brand_persons_file_name_prefix(brand_id):
    return os.path.join(CONTEXT, "brand_%s" % brand_id)


def get_brand_persons_file_name(brand_id):
    return os.path.join(smotrim.get_data_path(None), "%s_%s_%s.json" % (get_brand_persons_file_name_prefix(brand_id),
                                                                        CONTEXT_LIMIT, 0))


def get_persons_path(data_path=""):
    if not data_path:
        data_path = smotrim.get_data_path(None)
    return kodiutils.create_folder(os.path.join(data_path, CONTEXT))


def get_brand_persons_from_cache(brand_id):
    if brand_id:
...
def get_person_remote_thumbnail_url(brand_id, person_name):
    ...
    if m:
        ...
        xbmc.log("persons.get_person_remote_thumbnail_url found %s persons" % len(persons),
                 xbmc.LOGDEBUG)
        try:
            sp = list(filter(lambda p: p.get('name', '').encode('utf-8') == first_name and
...
def get_person_thumbnail(brand_id, person_name):
    return get_url("http://%s:%s/brands/%s" % (smotrim.SERVER_ADDR,
                                               smotrim.SERVER_PORT,
                                               brand_id), person_name=person_name.encode('utf-8'))
