    ...
    def get_channel_live_double(self, channel_id):
        ...
        if ch:
            ...
            if ch.get('type') == "vitrina":
                xbmc.log("Getting live stream for channel %s from Vitrina TV" % ch.get('title', '').encode('utf-8'),
                         xbmc.LOGDEBUG)
                return doublemap, self.get_vitrina_live_url(ch, headers)
            elif ch.get('type') == "video":
                xbmc.log("Getting live stream for channel %s from Smotrim.ru" % ch.get('title', '').encode('utf-8'),
                         xbmc.LOGDEBUG)
                ...
                xbmc.log("Getting live stream for radio %s from Smotrim.ru" % ch.get('title', '').encode('utf-8'),
                         xbmc.LOGDEBUG)
